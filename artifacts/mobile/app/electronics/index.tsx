...
import { MOCK_ELECTRONICS } from "@/constants/mockData";
import { useColors } from "@/hooks/useColors";
import { formatPrice } from "@/lib/format";
import type { ElectronicsCategory, ElectronicsItem } from "@/types";

const BRAND_COLOR = "#6366F1";

const CAT_FILTERS: { id: ElectronicsCategory | "all"; label: string; icon: keyof typeof Ionicons.glyphMap }[] = [
  { id: "all",         label: "Tout",        icon: "grid-outline" },
  { id: "phone",       label: "Téléphones",  icon: "phone-portrait-outline" },
  { id: "laptop",      label: "Ordinateurs", icon: "laptop-outline" },
  { id: "tablet",      label: "Tablettes",   icon: "tablet-portrait-outline" },
  { id: "tv",          label: "TV",          icon: "tv-outline" },
  { id: "audio",       label: "Audio",       icon: "headset-outline" },
  { id: "accessories", label: "Accessoires", icon: "hardware-chip-outline" },
];
...
function ElecCard({ item, onPress }: { item: ElectronicsItem; onPress: () => void }) {
  ...
  return (
    ...
    >
      ...
      <View style={styles.cardBody}>
        <Text style={[styles.cardBrand, { color: BRAND_COLOR }]}>{item.brand}</Text>
        <Text style={[styles.cardTitle, { color: colors.foreground }]} numberOfLines={2}>{item.title}</Text>
        {(item.storage || item.ram) && (
          <View style={styles.specRow}>
            {item.storage && (
              <View style={styles.specChip}>
                <Ionicons name="server-outline" size={10} color={colors.mutedForeground} />
                <Text style={[styles.specText, { color: colors.mutedForeground }]}>{item.storage}</Text>
              </View>
            )}
            {item.ram && (
              <View style={styles.specChip}>
                <Ionicons name="flash-outline" size={10} color={colors.mutedForeground} />
                <Text style={[styles.specText, { color: colors.mutedForeground }]}>{item.ram} RAM</Text>
              </View>
            )}
          </View>
        )}
...
export default function ElectronicsScreen() {
  ...
  const [cat, setCat] = useState<ElectronicsCategory | "all">("all");

  const topPad = Platform.OS === "web" ? 67 + 16 : insets.top + 16;
  const bottomPad = Platform.OS === "web" ? 34 + 84 : 100;

  const filtered = MOCK_ELECTRONICS.filter((i) => {
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { paddingTop: topPad, backgroundColor: BRAND_COLOR }]}>
        ...
        <View style={styles.headerText}>
          ...
          <Text style={styles.headerSub}>{MOCK_ELECTRONICS.length} annonces · Téléphones, ordis, TV…</Text>
        </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: bottomPad }}>
        ...
        <View style={styles.statsRow}>
          {[
            { icon: "phone-portrait" as const, val: MOCK_ELECTRONICS.filter((e) => e.category === "phone").length, lbl: "Téléphones" },
            { icon: "laptop" as const, val: MOCK_ELECTRONICS.filter((e) => e.category === "laptop").length, lbl: "Ordis" },
            { icon: "star" as const, val: MOCK_ELECTRONICS.filter((e) => e.featured).length, lbl: "Vedettes" },
          ].map((s) => (
            ...
            return (
              <Pressable
                ...
                style={[
                  styles.filterChip,
                  {
                    backgroundColor: active ? BRAND_COLOR : colors.muted,
                    borderColor: active ? BRAND_COLOR : colors.border,
                  },
                ]}
              >
        ...
        <View style={styles.results}>
          ...
          {filtered.map((item) => (
            <ElecCard
              key={item.id}
              item={item}
              onPress={() => router.push(`/electronics/${item.id}` as any)}
            />
          ))}
          {filtered.length === 0 && (
