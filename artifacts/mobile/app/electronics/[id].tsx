...
export default function ElectronicsDetailScreen() {
  const colors = useColors();
  const insets = useSafeAreaInsets();
  const { id } = useLocalSearchParams<{ id: string }>();
  const [imgIdx, setImgIdx] = useState(0);

  const item = MOCK_ELECTRONICS.find((e) => e.id === id);
  const similar = item
    ? MOCK_ELECTRONICS.filter((e) => e.id !== id && (e.category === item.category || e.city === item.city)).slice(0, 3)
    : [];
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: bottomPad }}>
        ...
        <View style={{ position: "relative" }}>
          ...
          <Pressable
            onPress={() => router.back()}
            style={[styles.backBtn, { marginTop: topPad + 12 }]}
          >
        ...
        <View style={styles.content}>
          ...
          <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <Text style={[styles.brand, { color: BRAND_COLOR }]}>{item.brand}</Text>
            ...
            <View style={[styles.specGrid, { borderTopColor: colors.border }]}>
              {[
                ...
                item.storage ? { icon: "server-outline" as const, label: "Stockage", val: item.storage } : null,
                item.ram ? { icon: "flash-outline" as const, label: "RAM", val: item.ram } : null,
                item.warranty ? { icon: "shield-checkmark-outline" as const, label: "Garantie", val: item.warranty } : null,
          ...
          {similar.length > 0 && (
            <View>
              <Text style={[styles.sectionTitle, { color: colors.foreground, paddingHorizontal: 0, marginBottom: 10 }]}>
                Annonces similaires
              </Text>
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginHorizontal: -4 }}>
                {similar.map((s) => (
                  <Pressable
                    key={s.id}
                    onPress={() => router.push(`/electronics/${s.id}` as any)}
                    style={({ pressed }) => [
                      styles.simCard,
      ...
      <View style={[styles.cta, { backgroundColor: colors.background, borderTopColor: colors.border, paddingBottom: insets.bottom + 12 }]}>
        <Pressable onPress={handleMessage} style={[styles.ctaSecondary, { borderColor: BRAND_COLOR }]}>
...
const styles = StyleSheet.create({
  ...
  brand: { fontSize: 12, fontWeight: "700", letterSpacing: 0.6, textTransform: "uppercase" },
