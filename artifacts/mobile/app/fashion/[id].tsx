...
const CAT_LABELS: Record<string, string> = {
  women: "Femmes", men: "Hommes", children: "Enfants",
  accessories: "Accessoires", shoes: "Chaussures", traditional: "Traditionnel",
};
...
export default function FashionDetailScreen() {
  ...
  const item = MOCK_FASHION.find((f) => f.id === id);
  const similar = item
    ? MOCK_FASHION.filter((f) => f.id !== id && (f.category === item.category || f.city === item.city)).slice(0, 3)
    : [];
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: bottomPad }}>
        ...
        <View style={styles.content}>
          ...
          <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
            {item.brand && <Text style={[styles.brand, { color: BRAND_COLOR }]}>{item.brand}</Text>}
            ...
            <View style={[styles.specGrid, { borderTopColor: colors.border }]}>
              {[
                ...
                item.size ? { icon: "resize-outline" as const, label: "Taille", val: item.size } : null,
                item.color ? { icon: "color-palette-outline" as const, label: "Couleur", val: item.color } : null,
          ...
          {/* Seller */}
          <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Vendeur / Créateur</Text>
            ...
            <View>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginHorizontal: -4 }}>
                {similar.map((s) => (
                  <Pressable
                    key={s.id}
                    onPress={() => router.push(`/fashion/${s.id}` as any)}
                    style={({ pressed }) => [
                      styles.simCard,
